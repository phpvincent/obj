...
<script>
    ...
    var a={!!$goods_config_arr!!}
        ...
        // console.log(i,val);
         ...
         addhtml='<form id="'+e+'"><div><strong>第'+eNum+'点</strong></div'+ color25+'</form>';   //每件商品的所有属性的HTML放入一个form；
                    ...
                    if(msg.goods.goods_cuxiao_type=="0"){
                        ...
                        $(function(){
                            ...
                            $("#addcart").append(specialHtml);
                             var yixuanHtml='<div class="addcart-specs-title unfold"><span class="addcart-specs-title-name">第'+msg.cuxiao[0].cuxiao_config.split(",")[0]+'点</span><span class="addcart-specs-arrow"></span><span class="addcart-specs-descript">（{{\App\currency_type::where('currency_type_id',$goods->goods_currency_id)->first()['currency_type_name']}}<span id="realprice">'+msg.cuxiao[0].cuxiao_config.split(",")[1]+'</span>, が選択された 【<span id="sell_msg">'+msg.cuxiao[0].cuxiao_msg+'</span>】 '+msg.goods.goods_num+'点が残っている）</span><span class="addcart-specs-status"></span></div>'
	                       ...
	                       $('#addcart-quantity-inc').bind('click',function(){
                            ...
                            $('.addcart-float-buttons-block').children('button').click(function(){    	
                            	...
                            	if(attr=='chose_cart'){
                                   ...
                                   $('.addcart-specs-title-name').html("第"+num+"点");
		                        console.log(num);
		                        $("#goods_config_div").children("form").remove(); //如果选择套餐先删除说有属性，在根据有几件商品循环几组属性；
		                        formnum=0;
		                        for(var i=1;i<=num;i++){
