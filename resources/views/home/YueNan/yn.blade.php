...
@if(in_array('delivery_place',$templets))
    ...
    <ul class="bannerq">
        ...
        <li class="bannerqli">Hình ảnh</li>
    </ul>
</div>
@endif
...
<div class="detail-context" style="border-bottom: 1px dashed #dcdcdc;padding:10px 2px;height:50px;">
    ...
    <span class="flag" style="font-size: 12px;">Miễn phí vận chuyển</span>
    @endif
    @if(in_array('cash_on_delivery',$templets))
    <span class="flag" style="font-size: 12px;">Thanh toán sau khi nhận hàng</span>
    @endif
    @if(in_array('seven_days',$templets))
    <span class="flag" style="font-size: 12px;background:#000;">có 7 ngày giám định hàng</span>
    @endif
</div>
@endif
...
@if(in_array('count_down',$templets))
    <div class="timebox">
        <div class="text">đếm ngược ：<span>{{$goods->goods_num}}nhóm　</span></div>
        <div class="boxtime">
            <div class="time" id="timer"><span id="h" class="colon"></span>tiếng<span id="m" class="colon"></span>phút<span id="s" class="colon"></span>giây </div>
            <font>cách kết thúc：</font>
        </div>
    </div>
@endif
...
<ul class="detail-bars">
    ...
    <li style="width:{{$center_nav==1 ? '100%' : ($center_nav==2 ? '50%' : '') }};">
        ...
        đánh giá ({{$goods->goods_comment_num}}+)
        </span>
    </li>
    @endif
</ul>
...
{{--            @if($goods->goods_comment_num!=0||$goods->goods_comment_num!=''||$goods->goods_comment_num!=null)--}}
    ...
    <tbody>
        ...
        <tr class="first odd">
            <td colspan="2">
                <p>Hiệu quả sử dụng thực tế của sản phẩm này dựa vào tình trạng cá nhân, không thể bảo đảm mỗi người sử dụng đều hưởng thụ được hiệu quả tuyên truyền. Nếu có vấn đề gì xin vui lòng tư vấn phục vụ khách hàng trên mạng hoặc liên hệ chúng tôi qua email(
        ...
        <tr class="first odd">
            <td colspan="2">
                <p>1.tình trạng trả hàng và đổi hàng vì nguyên nhân cá nhân : trong 7 ngày từ ngày nhận hàng, mà tình hình không ảnh hưởng tiêu thụ lần hai, xin liên hệ phục vụ khách hàng của chúng tôi hoặc gửi emaik đến
                    <a href="mailto:hyfhdcjn@gmail.com" style="color:#F8770E">hyfhdcjn@gmail.com</a>
                    phục vụ sau khi bán hàng sẽ trong1- 3 ngày nhận được tin tức xử lý yêu cầu của quý vị, phí vận chuyển của trả hàng và đổi hàng do khách hàng chịu. 
                </p>
                <p>
                   2.tình trạng trả hàng và đổi hàng vì nguyên nhân chất lượng của hàng hóa: trong 7 ngày từ ngày nhận hàng, gửi email cho trung tâm phục vụ sau khi bán hàng đến 
                                        <a href="mailto:hyfhdcjn@gmail.com" style="color:#F8770E">hyfhdcjn@gmail.com</a>,
        ...
        <tr> <td colspan="2">    
                <p>xác nhận hàng hóa -xin trả hàng và đổi hàng -phục vụ khách hàng xét duyệt thông qua -người sử dụng gửi về hàng hóa -kho ký nhận và nghiệm hàng-
Xét duyệt trả hàng và đổi hàng -trả tiền/đổi hàng. 

                </p>
                <p>Trả đổi hàng xin ghi rõ :số đơn đặt hàng, tên họ, số điện thoại. </p>
        ...
        <tr class="first odd">
           ...
           <td colspan="2">
                <p>Zsshop mall chọn lựa nghiêm khắc, luôn luôn tuân theo thái độ nghiêm cẩn ,thâm nhập khắp nơi thế giới, nghiêm khắc nắm chặt nơi sản xuất, công nghệ, nguyên tài liệu của hàng hóa, lựa chọn các loại hàng hóa gồm phục sức,giày dép và túi，ở nhà và vận động, hết sức cho quý khách hàng hóa chất lượng tốt nhất. 
               </p>
            </td>
                    ...
                    <table>
                        <tbody>
                            <tr>
                                ...
                                <td class="table_cell">
                                    <input type="text" placeholder="tên họ" class="mui-input-clear input01"
                                ...
                                <td class="table_cell">
                                    <input type="text" placeholder="số điện thoại " class="input01" name="phone"
                                    maxlength="20">
                            ...
                            <tr>
                                ...
                                <td class="table_cell">
                                    <textarea placeholder="dặn dò trên mạng" name="content" class="textarea_style"></textarea>
                                </td>
                            </tr>
                            <tr>
                                <td colspan="2" class="tc">
                                    <input id="btnAppraise" type="button" name="Submit" class="input_btn01"
                                    value="gửi  đáng giá" style="color:white">
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
<p style="margin: 0;height: 22px; background-color: #f5f5f5;display:none;"><span id="closeBtn" style="top:0;right:1px;position: absolute;z-index: 9;padding:0 14px;" class="mui-icon mui-icon-close"></span></p>
    ...
    <div class="btndiv1 mui-bar" style="box-shadow: 0px -1px 1px #dad8d8;max-width:640px;height:9%;display:none">
         <button id="pay" type="button" class="btnstyle01" style="width:100%;margin:0;height:100%">đưa ra đơn đặt hàng</button>
    </div>
</div>
...
//var time = endDate.getTime() - startDate.getTime();
            ...
            $("#timer").html('<span id="h" class="colon">' + h + '</span>'+"tiếng"+'<span id="m" class="colon">' + m + '</span>'+"phút"+'<span id="s" class="colon">' + s + '</span>'+"giây");
           time=time-1000;
...
*/
        ...
        function() {
            if ($2("input[name='name']").val() == '') {
                $2.toast("Tên họ không được làm chỗ trống ");
                return false;
            }
            if ($2("input[name='phone']").val() == '') {
                $2.toast("số điện thoại không được làm chỗ trống");
                return false;
            }
