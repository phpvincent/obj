...
<div class="addcart-group-buttons"  style="display: block;" ><div class="addcart-float-buttons-block"  data-id="7022">
	<button cuxiao_id='{{$v->cuxiao_id}}' @if($k==0) class='chose_cart'@else class='unchose_cart' @endif type="button" num='{{explode(",",$v->cuxiao_config)[0]}}' price='{{explode(",",$v->cuxiao_config)[1]}}' type_name='{{$v->cuxiao_msg}}' cuxiao_special_id='{{$v->cuxiao_special_id}}' >限時特惠 {{$v->cuxiao_msg}}[點擊購買]</button>
</div></div>
@endforeach
