...
@endforeach
<div class="addcart-specs-title unfold"><span class="addcart-specs-title-name">第1件</span><span class="addcart-specs-arrow"></span><span class="addcart-specs-descript">（{{\App\currency_type::where('currency_type_id',$goods->goods_currency_id)->first()['currency_type_name']}}<span id="realprice">998</span>，已選  <span id="sell_msg">{{$cuxiaos->first()->cuxiao_msg}}</span>  僅剩{{$goods->goods_num}}件）</span><span class="addcart-specs-status"></span></div>
...
<div class="addcart-footer"><div class="addcart-footer-price"><span class="addcart-footer-number-total">总数量:<font>{{explode(',',$cuxiaos->first()->cuxiao_config)[0]}}</font>，含贈 <font>0</font>件</span><span class="addcart-footer-price-total">合計:<font>{{\App\currency_type::where('currency_type_id',$goods->goods_currency_id)->first()['currency_type_name']}}{{explode(',',$cuxiaos->first()->cuxiao_config)[1]}}</font></span></div></div>
<script type="text/javascript">
			...
			@if($cuxiaos->first()->cuxiao_special_id!=null)
	...
	$('#addcart-quantity-dec').bind('click',function(){
		...
		pricehtml.html("{{\App\currency_type::where('currency_type_id',$goods->goods_currency_id)->first()['currency_type_name']}}"+(num-1)*price);
	})
