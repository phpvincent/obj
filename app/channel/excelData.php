...
//    }
    ...
    {
//       $cellData[] = ['数量(为必填项，货物实际数量)','运费付款方式(为必填项)','到付运费金额','到付运费币种','转单号码','订单编号(为必填项且小于20给字符)','件数(为必填项，小包数量)','包装类型(为必填项)','收费实重','收费材重','指定路线','目的地(为必填项)','中文品名','英文品名(为必填项)','申报价值(为必填项)','申报币别(为必填项)','寄件公司','寄件人(为必填项)','寄件人电话','寄件人手机','寄件地址(为必填项)','寄件国家','寄件省','寄件城市','寄件邮编','收件人(为必填项)','收件人电话(为必填项)','收件地址(为必填项)','收件省','收件城市(为必填项)','收件地区','收件邮编(为必填项)','代收款','代收款币别','货物类型(为必填项)','赠品'];
//        foreach($data as $k => $v) {
//            $exdata = [];
//            //数量(为必填项，货物实际数量)
...
//
//            //赠品
//            $exdata[36] = price::where('price_id',$v['order_price_id'])->value('price_name');
//
//            array_push($cellData,$exdata);
//        }
       $cellData[] = ['订单编号','运单编号','收件人','收货地邮编','收件人电话','收件省份','收件城市','收件地区','收件详细地址','重量KG','体积','代收货款','保价费RMB','货物类型','备注','中文品名','英文品名','件数','申报价值','申报价值币别','物品分类1','物品分类2','物品分类3','物品描述','URL','目的国家','订单类型'];
       foreach($data as $k => $v) {
            $exdata = [];

            //订单编号
            $exdata[1] = $v['order_single_id'];

            //运单编号
            $exdata[2] = "";

            //收件人
            $exdata[3] = $v['order_name'];
            ...
            if ($v['order_zip']) {
                ...
                $area_info = (isset($p[1][0]) && $p[1][0]) ? $p[1][0] : $v['order_add'];
                //收货地邮编
                $exdata[4] = $v['order_zip'];
                //收件人电话
                $exdata[5] = $v['order_tel'];
                //收件省
                $exdata[6] = $v['order_state'];
                //收件城市(为必填项)
                $exdata[7] = $v['order_city'];
                //收件地区
                $exdata[8] = $v['order_village'];
                //收件详细地址
                $exdata[9] = $v['order_state'] . ' ' . $v['order_city']. ' ' . $v['order_village'] . '(' . $area_info . ')';
            } else {
                $str = $v['order_add'];
                $pattern = '/(.*)\(Zip:(.*?)\)/';
                preg_match_all($pattern, $str, $p);
           ...
           //件数
           $exdata[18] = $v['order_num'];

           //申报价值
           $exdata[19] = $v['order_price'];
